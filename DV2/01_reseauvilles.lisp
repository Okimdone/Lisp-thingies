...
;;; une fonction Lisp qui retourne les vols directs (ou indirects) entre deux villes V1 et V2
;;; since the question is asking for all the paths, then i'm assuming there would be no cycles in the graph
;;; makes a graph node just which contains the reference to the "vol" and the path which we took to reach it
...
(defun get-actions (villeS &optional path (duree 0) (distance 0))
    (let ((action-nodes '())) 
        (dolist (vol (select-by-villeS VilleS))
            (push (make-node vol villeS path (+ duree (getf vol :Temps)) (+ distance (getf vol :Distance))) 
                action-nodes))
        action-nodes))
        ...
        (let ((node (pop stack)))
            (let ((reached-city (getf (car node) :villed)) 
                ...
                (if (equal VilleD reached-city)
                    (push (list (reverse (cons VilleD cached-path)) cached-duree cached-distance ) paths)
                    (dolist (n (get-actions reached-city cached-path cached-duree cached-distance))
                        (push n stack)))))))