                            ...
                            (do ((d nil (pop donnee)) (first-iter T))
                                ...
                                (let ((unified-symbols (unifier donnee pattern)))
                                    (cond ((null unified-symbols)) ;;; do nothing / continue
                                        ((equal T unified-symbols) (return (list (list p (reverse matched))))) 
                                        ;;; If the recursive call of the "unifier" returned a result of the form ((X (1 2 3))) merge with it and return
                                        (t (return (cons (list p (reverse matched)) unified-symbols)))))))
